...
#define TICK_NUM 100
#define SPECIFIC_BREAKPOINT_ADDRESS   0x80001000  // 设置触发断点异常的特定地址
#define SPECIFIC_ILLEGAL_INSTRUCTION_ADDRESS  0x80002000  // 设置触发非法指令异常的特定地址
...
void duandian_function() {
    // Insert an illegal instruction
    __asm__ volatile (".word 0xfeedbeef");  //断点异常 
}
...
static inline void trap_dispatch(struct trapframe* tf) {
    if ((intptr_t)tf->cause < 0) {
        ...
        //cprintf("EPC value before check: 0x%x\n", tf->epc);
        // Check for the condition to trigger breakpoint exception
